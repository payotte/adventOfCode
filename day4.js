...
const toPassphrases = (input) => input.split(/\n/).map((passphrase) => passphrase.split(' '));

const normalize1 = (word) => word;
const normalize2 = (word) => word.split("").sort().join("");

const isValid = (passphrase, normalize)  => {
    const counter = {};
    passphrase.forEach((word) => counter[normalize(word)] = (counter[normalize(word)] || 0) + 1);
    return !Object.values(counter).some((count) => count !== 1);
}

const numbersOfValidPassphrases = (input, normalize) => {
    const passphrases = toPassphrases(input);

    return passphrases.reduce((numberValid, passphrase) => numberValid + (isValid(passphrase, normalize) ? 1 : 0), 0)
};

console.log("Answer part 1: " + numbersOfValidPassphrases(input, normalize1));
console.log("Answer part 2: " + numbersOfValidPassphrases(input, normalize2));
